...
def fetch_news_from_prosearch(keyword):
    ...
    try:
        PORT = 19000
        from_time = int(__import__('time').time()) - 86400  # 最近24小时
...
for kw in keywords:
    ...
    if result:
        news_results[kw] = result[:500]  # 截取前500字
...
s1_html = section('策略① 放量上涨', '放量上涨 + 均线多头 + 停机坪 + 突破平台',
    ...
    '按换手率排序，取TOP25（排除688）', s1, ['code','name','pct','amount','turnover','inflow'], 25)

s2_html = section('策略② 成交额排名', '换手率排名 + 涨停板封单监控 + 大单交易追踪',
    '换手率 + 成交量 + 封单金额 + 大单净买入',
    '按成交额排序，取TOP25（排除688）', s2, ['code','name','amount','turnover','pct','inflow'], 25)

s3_html = section('策略③ 多因子量化', '中低频多因子选股 + 估值 + 成长 + 全市场分析',
    '市盈率PE + 市净率PB + 净资产收益率ROE + 净利润增长率',
    '筛选: 涨幅≥3% 且 成交额≥2亿（排除688）', s3, ['code','name','pct','amount','turnover','inflow'], 25)

s4_html = section('策略④ AI技术面', 'AI大模型K线技术指标分析 + 市场情绪分析',
    'KDJ金叉 + MACD金叉 + RSI + CCI + 量价配合',
    '筛选: 涨幅>0% 且 换手率>1% 且 主力净流入>0（排除688）', s4, ['code','name','inflow','pct','amount','turnover'], 25)

s5_html = section('策略⑤ 目标价+机构', '目标价上涨空间 + 董监高交易追踪 + 机构持仓变动',
    '目标价偏离度 + 董监高净买入 + 机构持股比例 + 北向资金',
    '筛选: 主力净流入≥1亿（排除688）', s5, ['code','name','inflow','pct','amount','turnover'], 25)
...
<div style="background:#f0f4f8;padding:12px;border-radius:6px;font-size:13px;line-height:1.8;color:#333;">
  ...
  <p><b style="color:#3498db">涨停情况:</b> 午间44只涨停，6只跌停 | 连板股：舒华体育8天5板、新能泰山6天5板、神剑股份4连板</p>
  <p><b style="color:#3498db">热点板块:</b></p>
  <ul style="margin:5px 0;padding-left:20px;">
    <li>🚗 <b>汽车整车</b>：宇通客车涨停，板块活跃</li>
    <li>🏗️ <b>盾构机/高铁</b>：中铁工业、中化岩土涨停，沪渝蓉沿江高铁加速推进</li>
    <li>🚀 <b>商业航天</b>：力箭二号首飞成功，神剑股份4连板</li>
    <li>🏋️ <b>体育产业</b>：舒华体育8天5板，产业链活跃</li>
  </ul>
  <p><b style="color:#3498db">政策面:</b> 央行继续实施适度宽松货币政策，强化逆周期和跨周期调节，社会融资成本处于历史较低水平</p>
  <p><b style="color:#3498db">经济数据:</b> 3月制造业PMI为50.4%，升1.4个百分点，制造业景气水平回升</p>
...
<div class="container">
  <div class="header">
    <h1>📊 五大策略综合选股报告</h1>
    <p>2026-03-31（周二）收盘 &nbsp;|&nbsp; 排除688 &nbsp;|&nbsp; 数据来源：东方财富、新浪财经、同花顺、财经网、第一财经</p>
  </div>
