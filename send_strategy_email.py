...
# 5组数据
data1 = fetch(get_url('f8', 100))  # 换手率
data2 = fetch(get_url('f6', 100))  # 成交额
data3 = fetch(get_url('f3', 200))  # 涨幅
data4 = fetch(get_url('f3', 200))  # AI技术面
data5 = fetch(get_url('f62', 100)) # 主力净流入

s1 = clean_stocks(data1)  # 换手率排序
s1.sort(key=lambda x: x['turnover'], reverse=True)

s2 = clean_stocks(data2)  # 成交额排序
s2.sort(key=lambda x: x['amount'], reverse=True)

s3 = clean_stocks(data3, min_pct=3.0)  # 涨幅>3%
...
s5 = clean_stocks(data5, min_inflow=1.0)  # 净流入>1亿
...
<html><head><meta charset="utf-8"><style>
  ...
  .disclaimer {{ color:#e94560; font-size:12px; margin-top:15px; }}
</style></head><body>
<div class="container">
  <div class="header">
    <h1>📊 五大策略综合选股报告</h1>
  ...
  <div style="padding:20px 25px">
    ...
    {s5_html}
    {rank_html}
    <p class="disclaimer">⚠️ 仅为数据扫描，不构成投资建议。止损原则：股票-3%无条件出。</p>
  </div>
  <div class="footer">
    由 Marcus（A股动量策略师）自动生成 | 仅供个人参考
  </div>
</div></body></html>'''
